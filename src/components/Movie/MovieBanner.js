...
const MovieBnner=function ({isLoading,data}) {
    if(isLoading){
        return <Loading/>
    }
    const  directorList=data.directors.map(function (item,index) {
    ...
    const actorList=data.casts.map(function (item,index) {
        if(index===data.directors.length-1){
            return item.name;
        }else{
            return item.name+"、"
